use e8_mask_codec::runtime::{
    Backend, is_prime_big,
    is_prime_fast, is_prime_proven,
    detect_prover,
    FastPrimeResult, ProvenPrimeResult,
    PrimeQueryConfig,
...
fn main() -> Result<(), String> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--prove" => prove = true,
            "--json" => json = true,
            "--progress" => { /* progress to stderr is always on for long computations */ }
            "--rounds" => {
                rounds = Some(
                    args.next()
            ...
            "--help" | "-h" => {
                println!("is-prime-big --n <N> [options]");
    ...
    if prove {
        ...
        if !prover.is_available() {
            if json {
                ...
                eprintln!("Falling back to fast (probabilistic) check.");
            }
        }
            ...
            ProvenPrimeResult::Composite { .. } => {
                if json {
                    let out = serde_json::to_string(&JsonResult {
                        ...
                        status: "composite".into(),
                        method: "mask+miller-rabin".into(),
                    ...
                    println!("n={n_preview}");
                    println!("is_prime=false");
                    println!("status=composite");
                    println!("method=mask+miller-rabin");
                    println!("elapsed_ms={elapsed_ms:.1}");
                }
            }
            ProvenPrimeResult::ProvenPrime { certificate } => {
                let snippet = if certificate.raw.len() > 200 {
                    format!("{}…", &certificate.raw[..200])
                } else {
                    certificate.raw.clone()
                };
                if json {
                    let out = serde_json::to_string(&JsonResult {
                    ...
                    println!("n={n_preview}");
                    println!("is_prime=true");
                    println!("status=proven_prime");
                    println!("method=mask+miller-rabin+ecpp");
                    println!("certificate={}", &certificate.raw[..certificate.raw.len().min(200)]);
                    println!("elapsed_ms={elapsed_ms:.1}");
                }
            }
            ProvenPrimeResult::ProbablePrime { rounds, prover_error } => {
                if json {
                    let out = serde_json::to_string(&JsonResult {
                        ...
                        method: format!("mask+miller-rabin ({rounds} rounds)"),
                        certificate_snippet: None,
                        elapsed_ms,
                    }).unwrap();
                    println!("{out}");
                    ...
                    println!("n={n_preview}");
                    println!("is_prime=true (probable)");
                    println!("status=probable_prime");
                    println!("method=mask+miller-rabin ({rounds} rounds)");
                    println!("ecpp_error={prover_error}");
                    println!("elapsed_ms={elapsed_ms:.1}");
                }
            }
        }
        ...
        let result = if n_digits > 19 {
            let fast = is_prime_fast(&n, &cfg, None);
            match fast {
                FastPrimeResult::Composite { .. } => (false, "composite", "mask+miller-rabin"),
                FastPrimeResult::ProbablePrime { .. } => (true, "probable_prime", "mask+miller-rabin"),
            }
        } else {
            let is_p = is_prime_big(&n, backend, None);
            let status = if is_p { "probable_prime" } else { "composite" };
            (is_p, status, "e8_mask")
        };
        let elapsed_ms = started.elapsed().as_secs_f64() * 1000.0;
        let (is_prime, status, method) = result;

        if json {
            let out = serde_json::to_string(&JsonResult {
                n_preview, n_digits, is_prime,
                status: status.into(),
                method: method.into(),
                certificate_snippet: None,
                elapsed_ms,
